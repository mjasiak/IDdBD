...
begin
     UserId:= ID;
     DataConnection.Select('SELECT user_id as ''Numer u¿ytkownika'', user_login as ''Login'', user_name as ''Imiê'', user_lastname as ''Nazwisko'', card_id as ''Numer karty'' FROM Users WHERE user_id = '''+IntToStr(UserId)+''';');
     lbl_delete.Caption := 'Czy na pewno chcesz usun¹æ konto u¿ytkownika '+DataConnection.IsString('Login')+', ' + IntToStr(UserId) + ' (ID)';
